...
export default function AlternatifEditModal({ isOpen, onClose, nama, periode }) {
  const [alternatifName, setAlternatifName] = useState(nama);
  const [alternatifPeriode, setAlternatifPeriode] = useState(periode);
    ...
    <>
      ...
      >
        <div className="flex items-start justify-between p-4 border-b rounded-t">
          <h3 className="text-xl text-black font-semibold">Edit {nama}</h3>
        </div>
        <Modal.Body>
          <div className="space-y-6">
            ...
            <div>
              ...
              <TextInput
                id="periode"
                placeholder={periode}
                value={alternatifPeriode}
                onChange={(event) => setAlternatifPeriode(event.target.value)}
                required
                theme={{
                  field: {
                    input: {
